    ...
    */
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int i=1; i<=N; i++) {
            ...
            z = true;
            for(Integer value : cases) {
                int containsCount = 0;
                for(int j=1; j<=usageIndex; j++) {
                    if(getUsage(currentValue, K-usages[j]) && getUsage(value, K-usages[j])) containsCount++;
                    else break;
                }
    ...
    //이진수 자릿수 확인을 위해 비트 연산 활용
    public static boolean getUsage(int value, int digit) {
